...
module.exports = {
  ...
  async execute(channel, client) {
    ...
    const threshold = await db.get(`guilds.${guildId}.protection.threshold`) ?? config.protection.channelCreate.threshold;
    const interval = await db.get(`guilds.${guildId}.protection.interval`) ?? config.protection.channelCreate.interval;
    ...
    const count = trackAction(executor.id, 'channelCreate', interval);
    if (count >= threshold) {
    ...
    if (everyoneRole.permissions.has('Administrator')) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`User <@${executor.id}> has **created channels** ${count} times within one minute.`)
        .setThumbnail(executor.displayAvatarURL())
        .addFields(
          { name: 'Action', value: 'Channel Create', inline: true },
      ...
      // Log to db
      const alert = {
        message: `User <@${executor.id}> created channels ${count} times within one minute.`,
