...
module.exports = {
  ...
  async execute(member, client) {
    ...
    const threshold = await db.get(`guilds.${guildId}.protection.threshold`) ?? config.protection.guildMemberRemove.threshold;
    const interval = await db.get(`guilds.${guildId}.protection.interval`) ?? config.protection.guildMemberRemove.interval;
    ...
    const count = trackAction(executor.id, 'guildMemberRemove', interval);
    if (count >= threshold) {
      // Remove all roles from executor except @everyone
      let execMember = await guild.members.fetch(executor.id).catch(() => null);
      if (execMember) {
        const rolesToRemove = execMember.roles.cache.filter(role => role.id !== guild.id);
        await execMember.roles.remove(rolesToRemove).catch(() => { });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`User <@${executor.id}> has **kicked members** ${count} times within one minute.`)
      ...
      // Log to db
      const alert = {
        message: `User <@${executor.id}> kicked members ${count} times within one minute.`,
